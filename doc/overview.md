...
│                                                          │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────────┐   │
│  │ State    │ │ Chat     │ │ User     │ │ Proactive  │   │
│  │ Machine  │ │ Session  │ │ Commands │ │ Scheduler  │   │
│  └──────────┘ └──────────┘ └──────────┘ └────────────┘   │
│  ┌──────────┐ ┌──────────┐                                │
│  │ STT Pool │ │ Memory   │                                │
│  │          │ │ Events   │                                │
...
10. **状态机** — `kokoro/state_machine.py` 两级层次状态机统一管理所有模块的状态转换，替代早期分散的布尔标志和锁。详见 [state_machine.md](state_machine.md)
...
| 模块 | 文件 | 功能 |
|------|------|------|
| 状态机 | `kokoro/state_machine.py` | 两级层次状态机，事件驱动，线程安全，统一管理所有状态转换 |
...
| CLI 入口 | `cli.py` | 语音模式主流程，各模块编排 + 多线程协调 + 状态机集成 |
