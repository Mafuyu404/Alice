...
class DialogueOrchestrator:
    ...
    def _autonomy_hint_for_event(self, event: DialogueEvent) -> str:
        ...
        lines = [
            ...
            "- 无事发生时也不要默认发呆：人的注意会漂移，可能想起旧梗、转向群聊、搜索一个不懂的东西、私聊一句轻量的话，或继续观察。若内在叙事流的【行动倾向】已有这些冲动，不要用“没有新输入”压掉它。",
            f"- QQ群和私聊是{name}正在生活的社交现场；群友可以是当前自然对话对象，不要仅因{user_name}沉默就忽略活跃群聊。",
            f"- 私聊窗口或旧对话残留不等于{name}必须等{user_name}先开口，除非当前内在叙事流有具体理由这样选择。",
        ]
