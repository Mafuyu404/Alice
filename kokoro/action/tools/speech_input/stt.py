...
def resolve_input_device(enabled: bool, preferred_device: int | None = None) -> int | None:
    if not enabled:
        return None
...
def create_default_recognizer(config: dict, *, printer=print) -> sherpa_onnx.OnlineRecognizer:
    model_path = download_model(config.get("stt_model_dir", "models/stt"))
    printer("  [cli] Loading speech model...")
    return create_recognizer(
        model_path,
        Namespace(num_threads=4, hotwords="", hotwords_score=1.5, verbose=False),
    )
