"""Impulse — LLM-based proactive speech planner.
...
3. Execute plan items: idle-wait → generate proactive speech → re-plan
...
class ImpulsePlanner:
    def __init__(
        ...
        *,
        ...
        self.planning_model = str(section.get("planning_model", "") or model)
        self.screen_timeout = max(5, int(section.get("screen_timeout", 45)))
        self.max_consecutive = max(1, int(section.get("max_consecutive_proactive", 3)))
    ...
    def on_conversation_end(self) -> None:
        """Call after every conversation turn TTS_DONE (user or proactive)."""
        if not self.enabled:
            return
        self.cancel()
    ...
    def _plan_and_execute(self) -> None:
        try:
            ...
            # 4. Execute the first plan item only — after speech, on_conversation_end
            #    will be called and re-plan, so we only handle one item per invocation.
            item = items[0]

            # 5. Idle-wait for the delay
    ...
    def _call_planner(self, screen_result: str, memories: str) -> PlanTable:
        ...
        try:
            ...
            return PlanTable.from_llm_response(
                text,
                max_capacity=self.max_plans,
                min_capacity=self.min_plans,
            )
    ...
    def _execute_item(self, item: PlanItem) -> None:
        """Generate and speak a proactive utterance for a plan item.
        ...
        # Guard against excessive consecutive proactive speech
        ...
        self._cancel_slot[0] = proactive_cancel

        try:
            # Build messages: system prompt + history + proactive trigger
            ...
            # Proactive trigger message — tells the LLM what to talk about
            ...
            user_trigger = _prompts.get("impulse.trigger_user", "（系统主动搭话触发）")
            ...
            if self.tts_engine:
                while self.tts_engine.is_playing and not proactive_cancel.is_set():
                    time.sleep(0.1)
            ...
            # Re-plan after proactive speech
